...
class GraphDatabase(KnowledgeBase):
    ...
    def _execute_entity_query(self, query: Text, target) -> List[Dict[Text, Any]]:
        """
            ...
            with client.session(keyspace=self.keyspace) as session:
                with session.transaction().read() as tx:
                    ...
                    for concept_map in concept_maps:
                        entity = concept_map.map().get(target)
                     
                        entities.append(self._thing_to_dict(entity, tx))
    ...
    def _execute_attribute_query(self, query: Text, target) -> List[Any]:
        """
        ...
        with GraknClient(uri=self.uri) as client:
            with client.session(keyspace=self.keyspace) as session:
                with session.transaction().read() as tx:
                    ...
                    result_iter = tx.query(query)
                    # concepts = result_iter.collect_concepts()
                    concepts = [ans.map().get(target) for ans in result_iter]
